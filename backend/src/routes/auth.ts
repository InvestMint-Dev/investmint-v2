...
router.get('/:userId', async (req: Request, res: Response) => {
  try {
    const user = await User.findById(req.params.userId).select('-password -passwordResetToken')
...
router.put('/update-data-from/:userId', async (req: Request, res: Response) => {
  try {
    const { data_from } = req.body
    await User.findByIdAndUpdate(req.params.userId, { data_from })
...
router.post('/reset-password', async (req: Request, res: Response) => {
  try {
    ...
    if (!user) { res.status(400).json({ message: 'Token is invalid or expired' }); return }
