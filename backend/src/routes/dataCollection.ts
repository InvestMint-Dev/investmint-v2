...
function scoreETFs(questionnaire: any) {
  const riskMap: Record<string, number> = { conservative: 1, moderate: 2, aggressive: 3 }
  const horizonMap: Record<string, number> = { within_3m: 1, '3m_1y': 2, '1y_3y': 3, over_3y: 4 }
  ...
  return ETF_UNIVERSE.map((etf) => {
    let score = 0
    if (risk === 1) score += (etf.volatility < 5 ? 3 : etf.volatility < 12 ? 1 : 0)
    if (risk === 2) score += (etf.volatility < 10 ? 2 : etf.volatility < 16 ? 1 : 0)
    if (risk === 3) score += (etf.returns1y > 15 ? 3 : etf.returns1y > 8 ? 2 : 1)
    if (horizon <= 2) score += (etf.category === 'Cash' || etf.category === 'Fixed Income' ? 2 : 0)
    if (horizon >= 3) score += (etf.category === 'Canadian Equity' || etf.category === 'US Equity' ? 1 : 0)
...
router.get('/run-etf-script', async (req: Request, res: Response) => {
  try {
    const { userId } = req.query as { userId: string }
    const cached = await ETFRecommendation.find({ user_id: userId })
    if (cached.length > 0) {
      res.json({ data: cached })
      return
    }
    const questionnaire = await InvestingQuestionnaire.findOne({ user_id: userId })
    const etfs = scoreETFs(questionnaire)
    await ETFRecommendation.insertMany(etfs.map((e) => ({ ...e, user_id: userId })))
    res.json({ data: etfs })
  } catch {
